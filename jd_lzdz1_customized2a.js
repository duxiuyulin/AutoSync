...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      authorCodeList = [
        "9b73221930c745cb85f6c47652bd4d05",
        "9d4dd82831a543a4aca2fd62f7dd286d",
        "1a44e6003c6e4e00a0edc7d3efa01384",
      ];
      ...
      $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/jul/qixiFirst/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=${encodeURIComponent($.secretPin)}&shopid=${$.activityShopId}&sid=&un_area=`;
      await member();
      // await $.wait(1000);
      if ($.bean > 0) {
...
async function member() {
  ...
  if ($.token) {
    ...
    if ($.secretPin) {
      ...
      // console.log($.cookie)
      await task("jul/qixiFirst/activityContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=&nick=${encodeURIComponent($.pin)}&shareUuid=${encodeURIComponent($.authorCode)}`);
      $.log("关注店铺");
      await task("jul/qixiFirst/saveTask", `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=23&taskValue=&shareUuid=${encodeURIComponent($.authorCode)}`);
      await $.wait(1000);
      await task("jul/qixiFirst/saveTask", `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}&taskType=6&taskValue=6&shareUuid=${encodeURIComponent($.authorCode)}`);
      await $.wait(1000);
      await task("taskact/common/drawContent", `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}`);
      await $.wait(1000);
      await task("jul/qixiFirst/initOpenCard", `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&shareUuid=${encodeURIComponent($.authorCode)}`);
      $.log("加入店铺会员");
      if ($.openCardList) {
      ...
      await $.wait(1000);
      await task("jul/qixiFirst/draw", `pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}`);
            ...
            if (data.result) {
              switch (function_id) {
                ...
                case "wxActionCommon/getUserInfo":
                  break;
                case "jul/qixiFirst/activityContent":
                  // console.log(data)
                  if (!data.data.hasEnd) {
                  ...
                  break;
                case "jul/qixiFirst/initOpenCard":
                  $.openCardList = data.data.openInfo;
                  $.openCardStatus = data.data.allOpenCard;
                  // console.log(data)
                  break;
                case "jul/qixiFirst/saveTask":
                  console.log(data);
                  break;
                case "jul/qixiFirst/draw":
                  console.log(data);
                  break;
